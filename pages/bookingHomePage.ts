...
export class BookingHomePage extends BasePage {
  ...
  // Locators
  // -------------------------------
  private searchDestinationField = this.page.locator(
    'input[name="ss"], input[placeholder="Where are you going?"]'
  );
  private datePickerContainer = this.page.locator(
    'button[data-testid="searchbox-dates-container"]'
  );
  private searchButton = this.page.locator(
    'button[type="submit"]:has-text("Search")'
  );
  private dismissSignInButton = this.page.locator(
    'button[aria-label="Dismiss sign-in info."]'
  );
  private closePopupButton = this.page.locator('button[aria-label="Close"]');
  private nextMonthButton = this.page.locator(
  ...
  /** Start background popup watcher — runs silently during test */
  private startPopupWatcher() {
    const popupSelectors = [
      "button.js-dismiss-banner",
      'button[aria-label*="Close"]',
      'button:has-text("Dismiss")',
      'button:has-text("No thanks")',
      'button:has-text("Maybe later")',
      '[id*="close"], [class*="close"], [data-testid*="close"]',
    ];

    // Run async but don't block main flow
    (async () => {
      console.log("👀 Popup watcher started...");
      for (let i = 0; i < 10; i++) {
        ...
        await this.page.waitForTimeout(2000); // wait before next scan
      }
      console.log("🧹 Popup watcher finished");
    })();
  }
  ...
  async isHomePageLoaded(): Promise<boolean> {
    ...
    try {
      ...
      console.error(
        "❌ Home page did not load — destination field not found after 3 attempts"
      );
  ...
  private async selectDateByValue(date: string, dateName: string) {
    ...
    while (
      !(await dateLocator.isVisible().catch(() => false)) &&
      safetyCounter < 12
    ) {
      safetyCounter++;
      console.log(
        `   ➡️ Navigating to next month to find ${dateName}: ${date}`
      );
  ...
  /** Click Search button */
async clickSearch(): Promise<Page> {
  console.log("🔍 Clicking search button...");
  await this.searchButton.scrollIntoViewIfNeeded();
  await this.page.waitForTimeout(3000);

  // Use helper and capture new page
  ...
  async searchHotel(destination: string, checkIn: string, checkOut: string) {
    ...
    try {
      const autocomplete = this.page
        .locator(`li:has-text("${destination}")`)
        .first();
